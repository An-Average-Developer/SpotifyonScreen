...
namespace SpotifyOnScreen.Services
{
    ...
    public class UpdateService : IDisposable
    {
        ...
        public UpdateService()
        {
            ...
            _httpClient.DefaultRequestHeaders.Add("User-Agent", "SpotifyOnScreen");
        ...
        public async Task<UpdateInfo> CheckForUpdatesAsync()
        {
            ...
            try
            {
                ...
                {
                    var fileName = $"SpotifyOnScreen-v{latestVersion.Replace(".", "-")}.zip";
...
@echo off
chcp 65001 >nul
echo Updating Spotify On Screen...
...
";
        ...
        private string FindExecutable(string directory)
        {
            ...
            {
                "SpotifyOnScreen.exe",
                Path.GetFileName(GetCurrentExecutablePath()) ?? "SpotifyOnScreen.exe"
            };
        ...
        private string GetCurrentExecutablePath()
        {
            try
            {
                ...
                var exeName = Path.GetFileNameWithoutExtension(Environment.ProcessPath ?? "SpotifyOnScreen") + ".exe";
                return Path.Combine(baseDir, exeName);
            }
            catch
            {
                var baseDir = AppContext.BaseDirectory;
                return Path.Combine(baseDir, "SpotifyOnScreen.exe");
            }
        }
